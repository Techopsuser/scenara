...
export async function GET(req: Request) {
  ...
  const items = scenarios.map((s) => {
    ...
    return {
      ...
      hasAttachments: false, // Removed attachments logic, set to false
      attachmentsCount: 0,   // Removed attachments logic, set to 0
    }
  })
  ...
  try {
    ...
    const scenario = await db.scenario.create({
      data: {
        ...
        difficulty,
        authorId: userId,
        // attachments completely removed from here to fix Prisma Error
        technologies: {
